/** Shadow-only parser. Not imported by the plugin or QMD runtime. All offsets are UTF-16. */
export type Edit = { start: number; end: number; replacement: string; reason: string };
export type Proposal = { edits: Edit[]; preserved: { start: number; end: number }[] };
...
function stripRoute(text: string): string {
...
export function parseInternalMessage(text: string, trustedInterSession: boolean): Proposal {
  const unchanged = { edits: [], preserved: [] };
  if (!trustedInterSession || text.includes("\r")) return unchanged;
  const routed = stripRoute(text);
  const wrapped = routed.startsWith(INTERNAL);
  const prefix = wrapped ? INTERNAL : BACKGROUND;
  if (!routed.startsWith(prefix)) return unchanged;
  ...
  if (child < 0 || close < 0 || text.indexOf("<prompt-data>", child + CHILD.length) >= 0 ||
      text.indexOf("</prompt-data>", close + 2) >= 0) return unchanged;
  ...
  if (!tail.startsWith(expected)) return unchanged;
  tail = tail.slice(expected.length).trim();
  if (tail && stripRoute(tail + "\n").trim()) return unchanged;
...
function codeRanges(text: string): { start: number; end: number; closingStart?: number }[] {
  ...
  for (const line of lines) {
    ...
    if (!fence) continue;
    if (!open) open = { start: line.index, char: fence[1][0], length: fence[1].length };
...
function legacyHtml(text: string, offset: number): Proposal | undefined {
...
export function parseAttachments(text: string): Proposal {
  const result: Proposal = { edits: [], preserved: [] };
  ...
  for (const match of text.matchAll(pattern)) {
    ...
    if (fences.some(r => r.start < end && r.end > start) ||
        /<\/?file\b|EXTERNAL_UNTRUSTED_CONTENT/.test(match[4])) continue;
    ...
    if (html) { result.edits.push(...html.edits); result.preserved.push(...html.preserved); }
    else result.preserved.push({ start: bodyStart, end: bodyEnd });
  }
  return result;
}

/** Only a recognized source-level REM heading/marker span, never arbitrary short content. */
export function parseDreamingMarkers(text: string): Proposal {
  const fences = codeRanges(text);
  const edits: Edit[] = [];
  for (const m of text.matchAll(/^## REM Sleep\n<!-- openclaw:dreaming:rem:start -->\n/gm)) {
    const start = m.index, end = start + m[0].length;
    if (!fences.some(r => r.start < end && r.end > start)) edits.push({ start, end, replacement: "", reason: "dreaming-heading-marker" });
  }
  return { edits, preserved: [] };
}

/** A source-aware chunk eligibility check: do not change the original Markdown or its offsets. */
export function structuralChunkReason(content: string, start: number, end: number): string | undefined {
  const text = content.slice(start, end);
  if (parseDreamingMarkers(content).edits.some(e => e.start <= start && e.end >= end) && text.trim()) return "dreaming-heading-marker";
  // Only a closing fence of a known fenced block, with substantive content elsewhere in that block.
  if (/^\s*(?:`{3,}|~{3,})\s*$/.test(text)) {
    const range = codeRanges(content).find(r => r.end === end && r.closingStart === start && r.start < start);
    if (range && content.slice(range.start, start).split("\n").slice(1).join("\n").trim()) return "orphan-closing-fence";
  }
  return undefined;
}

export function applyProposal(text: string, proposal: Proposal): string {
  const edits = [...proposal.edits].sort((a, b) => a.start - b.start || a.end - b.end);
  let position = 0;
  let output = "";
  for (const edit of edits) {
    if (edit.start < position || edit.end < edit.start || edit.end > text.length) throw new Error("Invalid or overlapping edit");
    if (proposal.preserved.some(p => p.start < edit.end && p.end > edit.start)) throw new Error("Edit overlaps preserved payload");

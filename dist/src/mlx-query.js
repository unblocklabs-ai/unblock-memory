...
export function queryConversation(prompt, messages, historyMessages) {
    ...
    while (history.length && Buffer.byteLength(JSON.stringify({ history, currentRequest })) > 24_000)
        history.shift();
    if (!currentRequest)
        throw new Error("Missing or unparseable current request");
...
export class MlxQueryGenerator {
    ...
    static shared(config) {
        ...
        const workers = (scope[symbol] ??= new Map());
        const key = JSON.stringify([config.pythonPath, config.modelPath]);
        let worker = workers.get(key);
        if (!worker || worker.closed) {
            worker = new MlxQueryGenerator(config);
    ...
    start() {
        if (this.#closed)
            return Promise.reject(new Error("Query worker stopped"));
        if (this.#ready)
            return this.#ready;
        if (Date.now() < this.#retryAfter)
            return Promise.reject(new Error("Query worker restarting"));
        ...
        this.#ready = new Promise((resolve, reject) => {
            ...
            const fail = () => {
                ...
                for (const request of this.#pending.values())
                    request.reject(new Error("Query worker unavailable"));
                this.#pending.clear();
            };
            ...
            lines.on("line", line => {
                try {
                    ...
                    if (!request)
                        return; // Cancelled request; never surface late results.
                    this.#pending.delete(value.id);
                    if (!("text" in value) || typeof value.text !== "string") {
                        request.reject(new Error("Query generation failed"));
                        return;
                    }
                    let queries;
                    try {
                        queries = JSON.parse(value.text).queries;
                    }
                    catch { /* reject below */ }
                    ...
                    if (!usable.length) {
                        request.reject(new Error("No usable generated queries"));
                        return;
                    }
    ...
    async generate(conversation, signal) {
        ...
        if (this.#pending.size >= 8)
            throw new Error("Query worker busy");
        const id = randomUUID();
        ...
        const cancel = () => {
            ...
            if (child.stdin.writable)
                child.stdin.write(JSON.stringify({ id, cancel: true }) + "\n");
        };

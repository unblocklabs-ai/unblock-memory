...
type RefinementOutput = Static<typeof REFINEMENT_OUTPUT_SCHEMA>;
...
function evidenceKey(source: string, locator: string): string {
  return `${source}\0${locator}`;
}

function existingEvidence(dossier: PersonDossier | undefined): Set<string> {
  return new Set(
    dossier?.sections.flatMap((section) =>
      section.claims.flatMap((claim) =>
        claim.evidence.map((evidence) => evidenceKey(evidence.source, evidence.locator)),
      ),
    ) ?? [],
  );
}
...
export async function refinePeople(params: {
  ...
  const input: PeopleRefinementInput = { people: [] };
  ...
  for (const person of candidates) {
    ...
    const evidence =
      params.includeSessionEvidence === false
        ? []
        : identities
            ...
            .filter(
              (entry, index, all) =>
                all.findIndex((other) => other.locator === entry.locator) === index,
            )
    ...
    for (const item of evidence) known.add(evidenceKey(item.source, item.locator));
  ...
  const output: RefinementOutput = Value.Parse(REFINEMENT_OUTPUT_SCHEMA, rawOutput);
  ...
  if (
    resultsById.size !== output.results.length ||
    ...
    throw new Error(
      "Codex refinement output must contain exactly one result for every selected person",
    );
  }
  ...
  for (const person of input.people) {
    params.store.replaceDossier(
      person.personId,
      resultsById.get(person.personId)!.dossier,
      person.lastSeenAt,
      { requireRefinementEnabled: true },
    );
  }
...
function codexPrompt(input: PeopleRefinementInput): string {
  return [
...
const runCodexProcess: CodexCommandRunner = async (params) => {
  await new Promise<void>((resolve, reject) => {
    ...
    child.once("close", (code, signal) => {
      ...
      reject(
        new Error(
          `codex exec ${signal ? `was terminated by ${signal}` : `exited with code ${code ?? "unknown"}`}${detail ? `: ${detail}` : ""}`,
        ),
      );
    });
  });
...
function codexEnvironment(source: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  return Object.fromEntries(
    CODEX_ENV_KEYS.flatMap((key) => (source[key] === undefined ? [] : [[key, source[key]]])),
  );
}

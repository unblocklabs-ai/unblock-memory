...
export function meetingRevisionAnnotation(content: string, position: number): string | undefined {
  const enclosingMessage = content.lastIndexOf("\n## User — ", position);
  return /^Transcript revision \d+ \(superseded by revision \d+\)\.$/mu.exec(
    content.slice(Math.max(0, enclosingMessage), position),
  )?.[0];
}

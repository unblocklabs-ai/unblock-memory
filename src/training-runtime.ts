...
function thresholdOption(value: string) {
  const result = Number(value);
  if (!value.trim() || !Number.isFinite(result) || result < 0 || result > 1) throw new Error("Threshold must be between 0 and 1");
  return result;
}

/** CLI only: no scheduler, tools, hooks, or live memory-index changes. */
export function registerMemoryTraining(api: OpenClawPluginApi, config: UnblockMemoryConfig) {
  const paths = (cfg: OpenClawConfig, id: string) => {
    const normalized = normalizeAgentIdStrict(id);
    if (!normalized.ok || !listAgentIds(cfg).includes(normalized.value)) throw new Error("Unknown memory-training agent");
  ...
  api.registerCli(({ program, config: cfg }) => {
    ...
    const withStore = async <T>(agent: string, options: { create?: boolean; readOnly?: boolean }, fn: (store: TrainingStore, source: ReturnType<typeof paths>) => T | Promise<T>) => {
      const source = paths(cfg, agent);
      if (!options.create && !existsSync(source.storePath)) throw new Error("No training database; run memory-training collect first");
      const store = new TrainingStore(source.storePath, source.agentId);
      try { return await (options.readOnly ? fn(store, source) : store.locked(() => fn(store, source))); } finally { store.close(); }
    };
    root.command("collect").option("--agent <id>", "Agent id", "main")
      ...
      .action(async (opts: { agent: string; since?: string; until?: string; dryRun?: boolean }) => {
        const since = dateOption(opts.since, 0), until = dateOption(opts.until, Number.MAX_SAFE_INTEGER);
        if (until <= since) throw new Error("until must be later than since");
    ...
    root.command("run").description("Judge whether historical recall would help each collected input").option("--agent <id>", "Agent id", "main")
      ...
      .action(async (opts: { agent: string; maxExamples?: string; maxInputBytes: string; concurrency: string; dryRun?: boolean }) => {
    ...
    root.command("status").option("--agent <id>", "Agent id", "main")
      ...
      .action(async (opts: { agent: string; threshold: string }) => {
    ...
    root.command("export").option("--agent <id>", "Agent id", "main")
      ...
      .action(async (opts: { agent: string; threshold: string; output: string; stage: string }) => {
        if (!["recall-gate", "query-training"].includes(opts.stage)) throw new Error("Unknown training export stage");
        ...
        const result = await withStore(opts.agent, {}, (store, source) => {
          ...
          try { for (const row of data) { writeSync(fd, JSON.stringify(row) + "\n"); rows++; } }

...
export class ResponseReviews {
  ...
  sync(cohort: string, episodeId: string, inputHash: string, result: ResponseResult, now: number) {
    ...
    const agentDirected = ["current_answer", "earlier_behavior", "delivery", "proactive_action", "mixed"] as const;
  ...
  refresh(cohort: string, since: number) {
    ...
    try {
      ...
      for (const row of rows) this.sync(cohort, String(row.id), String(row.input_hash), JSON.parse(String(row.result)) as ResponseResult, Date.now());
      this.db.exec("COMMIT"); return rows.length;
    } catch (error) { this.db.exec("ROLLBACK"); throw error; }
  }
  reconcile(cohort: string) {
  ...
  list(cohort: string, id?: string) {
    return this.db.prepare(`SELECT t.*,e.input_hash,e.active,e.detail FROM response_review_tasks t
      ...
      evidence: JSON.parse(String(row.detail)) as unknown,
    }));
  }
  decide(cohort: string, id: string, status: ResponseReviewStatus, reviewer: "human" | "agent", note: string, now = Date.now()) {
    if (!["pending", "resolved", "dismissed", "deferred"].includes(status) || !["human", "agent"].includes(reviewer) ||
      !note.trim() || note.length > 4000) throw new Error("Invalid response review decision");
    if (!this.list(cohort, id).length) throw new Error("Unknown response review task in current cohort");
    this.db.exec("BEGIN IMMEDIATE");
    try {
  ...
  annotate(occurredAt: number, kind: string, note: string) {
    if (!Number.isFinite(occurredAt) || !["model", "prompt", "deployment", "other"].includes(kind) || !note.trim() || note.length > 4000) throw new Error("Invalid response annotation");
    const id = randomUUID();
    this.db.prepare("INSERT INTO response_annotations VALUES(?,?,?,?)").run(id, occurredAt, kind, note.trim());
    return id;
  }
  annotations(since: number, until: number) {

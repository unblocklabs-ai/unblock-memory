...
import {
  codexPeopleRefinementRunner,
  refinePeople,
  type PeopleRefinementRunner,
} from "./people-refinement.js";

export function registerPeopleCli(
  api: OpenClawPluginApi,
  config: UnblockMemoryConfig["people"],
  runner: PeopleRefinementRunner = codexPeopleRefinementRunner,
): void {
  api.registerCli(
    ({ program, config: openClawConfig }) => {
      ...
      people
        ...
        .action(async (options: { agent: string }) => {
          ...
          if (!agentId) throw new Error("--agent must be a non-empty string");
          if (!config.enabled) throw new Error("PeopleSQL is disabled");
          ...
          try {
            const summary = await refinePeople({
              ...
              agentId,
              agentDatabasePath: join(
                resolveAgentDir(openClawConfig, agentId),

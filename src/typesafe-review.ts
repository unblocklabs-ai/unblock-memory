...
export async function reviewMemoryRedundancy(params: RequestOptions & { excerpts: readonly string[] }) {
  ...
  return Promise.all(pairs.map(async ({ earlier, later }) => {
    ...
    if (!Value.Check(nouls, payload) || Object.keys(payload.answers).length !== 1 ||
        !Object.hasOwn(payload.answers, "pair_0")) throw new Error("TypeSafe returned invalid redundancy judgments");
...
export async function reviewClusterDefects(params: RequestOptions & { excerpts: readonly string[] }) {
  ...
  return Promise.all(params.excerpts.map(async excerpt => {
    ...
    if (!Value.Check(schema, payload) || Object.keys(payload.answers).length !== 1 ||
        !Object.hasOwn(payload.answers, "member_0")) throw new Error("TypeSafe returned invalid cluster judgments");

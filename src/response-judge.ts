...
function choiceSchema<T extends Record<string, unknown>>(criteria: T) {
  const labels = Object.keys(criteria) as (keyof T & string)[];
...
export type ResponseJudgment = { quality: ResponseQuality; feedback: ResponseFeedback };
...
function sentimentFields({ sentiment, annoyance, frustration, dissatisfactionIntensity }: ResponseStages["sentiment"]) {
...
function validateDistributions(answers: Record<string, { type: string; probabilities?: Record<string, number>; score?: number }>) {
...
export async function judgeResponse(episode: ResponseEpisode, params: { apiKey: string; timeoutMs: number; signal: AbortSignal }, sentimentEnabled = true, cache?: StageCache): Promise<ResponseJudgment> {
  ...
  if (!Value.Check(qualitySchema, qualityPayload)) throw new Error("Invalid response quality judgment");
  ...
  if (sentimentEnabled && !Value.Check(sentimentSchema, sentimentPayload)) throw new Error("Invalid response sentiment judgment");
  ...
  if (needFeedback) cache?.save("feedback", { feedbackType, target, avoidableRework, repeatedConstraint, memoryGap });
  ...
  if (sentimentEnabled && Value.Check(sentimentSchema, sentimentAnswers)) {
    ...
    if (needSentiment) cache?.save("sentiment", sentimentAnswers);
  }
...
export async function judgeResponseFollowup(episode: ResponseEpisode, params: { apiKey: string; timeoutMs: number; signal: AbortSignal }):
  ...
  if (!Value.Check(retrospectiveSchema, payload)) throw new Error("Invalid response follow-up judgment");

...
type Edit = { start: number; end: number; replacement: string; reason: string };
type Proposal = { edits: Edit[]; preserved: { start: number; end: number }[]; budgetSkipped?: boolean };
...
export function parseAttachments(text: string): Proposal {
  ...
  if (!text.includes('<file name="')) return result;
  if (text.length > MAX_ATTACHMENT_SCAN_WORK) return { ...result, budgetSkipped: true };
  const starts = /^<file name="/gm;
  let remainingWork = MAX_ATTACHMENT_SCAN_WORK;
  while (starts.exec(text)) {
    remainingWork -= text.length;
    if (remainingWork < 0) return { ...result, budgetSkipped: true };
  }

...
export async function resolveTypeSafeApiKey(config: UnblockMemoryConfig["typesafe"]): Promise<string | undefined> {
...
export async function requestTypeSafe(params: { apiKey: string; timeoutMs?: number; signal?: AbortSignal },
  ...
  try {
    ...
    if (!response.ok) {
      try { await response.body?.cancel(); } catch { /* Status is authoritative; discard unsafe cancellation detail. */ }
      throw new TypeSafeRequestError(`TypeSafe HTTP ${response.status}`, "http_error", response.status);
    }
    try { return await response.json(); }
    catch (error) {
      if (error instanceof SyntaxError) throw new TypeSafeRequestError("TypeSafe returned invalid JSON", "invalid_response");
      throw error;
    }
  } catch (error) {
    ...
    if (signal?.aborted) {
      ...
      throw new TypeSafeRequestError(timedOut ? "TypeSafe request timed out" : "TypeSafe request cancelled",

...
type ChunkRow = {
  document_id: number; seq: number; collection: string; path: string;
  hash: string; pos: number; chunk_len: number; doc: string;
};
const MAX_CHUNK_CHARS = 6000;

/** A formatting clue, never proof that JSON or structured data is worthless. */
export function qualityStructure(text: string): "empty" | "encoded_message" | "serialized_message" | "plain_or_structured" {
  if (!text.trim()) return "empty";
  let value: unknown;
  try { value = JSON.parse(text); } catch { return "plain_or_structured"; }
  const encoded = typeof value === "string";
  if (typeof value === "string") {
    try { value = JSON.parse(value); } catch { return "plain_or_structured"; }
  }
...
export async function auditQualityPage(params: {
  ...
  const check = () => {
    ...
    if (!params.isActive()) throw new Error("audit stopped");
  };
  check();
  if (!sources.size) return result("ok", true);
  ...
  const rows = db.prepare(`SELECT d.id AS document_id, cv.seq, d.collection, d.path,
    ...
    ORDER BY d.id, cv.seq LIMIT ?`).all<ChunkRow>(
    ...sources.keys(), params.after?.documentId ?? 0, params.after?.documentId ?? 0, params.after?.seq ?? -1, limit + 1,
  );
  ...
  try {
    ...
    check();
    const fresh = new Map<string, QualityJudgment>();
    for (const [index, answer] of answers.entries()) {
      if (answer.status !== "fulfilled") continue;
      const { cacheKey, row } = missing[index];
      if (!current.get(row.document_id, row.collection, row.path, row.hash, row.seq, row.pos, row.chunk_len)) continue;
    ...
    for (const item of batch) {
      ...
      if (!item.eligible || !parseSafeVirtualPath(`qmd://${source.collection}/${row.path}`, sources) ||
        ...
        skippedStale++; advance(); continue;
      }
      if (text.length > MAX_CHUNK_CHARS) { skippedOversized++; advance(); continue; }
      const judgment: QualityJudgment | undefined = structure === "empty"
        ? { noise: 1, evidence: 0 } : item.judgment ?? fresh.get(cacheKey);
      if (!judgment) throw new Error("missing quality judgment");
      if (item.judgment) cached++;
      if (structure !== "empty" && structure !== "encoded_message" && judgment.noise < params.minNoise) { advance(); continue; }
